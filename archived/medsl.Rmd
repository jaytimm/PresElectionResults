...
vvo <- Rvoteview::download_metadata(type = 'members', chamber = 'house') %>%
  filter(congress > 94 & chamber != 'President')
```
...
> In MEDSL, per every election winner (no specials) & per every year/state/district: find orthographically closest name from list of lawmakers in VoteView --
...
```{r eval=FALSE, include=FALSE}
#uspols_medsl_house <- ctable1
uspols_medsl_senate <- ctable1
setwd('/home/jtimm/jt_work/GitHub/packages/uspols')
#devtools::use_data(uspols_dk, overwrite=TRUE)
usethis::use_data(uspols_medsl_senate, overwrite=TRUE)
```
