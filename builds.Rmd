...
w3a <- c('New Mexico', 
         ...
         'Connecticut',
         'New York',
         'Washington')  # Connecticut, New York, and Washington use same column structure as w3a
w3c <- c('Pennsylvania')
w3d <- c('California')
w5 <- c('Nebraska')
w4a <- c('Maine')
...
states <- states_full |>
  ...
  mutate (          l1 = case_when (NAME %in% c(w3a, w6, w4b, w5) ~ list(list(1,3,4,5)), # Nebraska: Year (col1), Winner candidate (col3), votes (col4), % (col5)
          ...
          l3 = case_when (NAME == 'Connecticut' ~ list(list(1,10,11,12)),  # Connecticut: Year (col1), Other candidate (col10), votes (col11), % (col12)
                          ...
                          !NAME %in% c(w3a, w3c, w3d, w6, w4b, w5, w7, 'Connecticut', 'New York', 'Washington') ~ list(list(1,8, 9, 10)),
          ...
          ns = case_when (NAME == 'Connecticut' ~ 7,  # Connecticut uses table 7 - must come BEFORE w3a check
                          ...
                          NAME %in% c(w6) ~ 6,
                          !NAME %in% c(w3a, w3c, w3d, w4a, w4b, w5, w6, w7, 'Connecticut', 'New York', 'Washington') ~ 2)
  ) |>
